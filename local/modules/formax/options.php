...
$aTabs = array(
    array(
         ...
         */
        'DIV'     => 'edit1',
        'TAB'     => Loc::getMessage('FORMAX_OPTIONS_TAB_GENERAL'),
        'TITLE'   => Loc::getMessage('FORMAX_OPTIONS_TAB_GENERAL'),
        'OPTIONS' => array(
            array(
                'switch_on',                                   // имя элемента формы
                Loc::getMessage('FORMAX_OPTIONS_SWITCH_ON'), // поясняющий текст — «Включить прокрутку»
...
<form action="<?= $APPLICATION->GetCurPage(); ?>?mid=<?=$module_id; ?>&lang=<?= LANGUAGE_ID; ?>" method="post">
    ...
    <input type="submit" name="apply"
           value="<?= Loc::GetMessage('FORMAX_OPTIONS_INPUT_APPLY'); ?>" class="adm-btn-save" />
    <input type="submit" name="default"
           value="<?= Loc::GetMessage('FORMAX_OPTIONS_INPUT_DEFAULT'); ?>" />
    </form>
